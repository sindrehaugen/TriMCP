...
1. One batch = one branch = one commit. Branch `batch-96-packaging-proxmox`. Never combine batches.
...
8. `NCE_MASTER_KEY` is env-only — never read/write it via DB/settings/endpoint. The appliance must NOT bake secrets into the image; the first-boot wizard supplies them.
...
- C4 Diff: `git add -A` → write `git diff --cached` to `_internal/diffs/diff_batch_96-packaging-proxmox.md`; set this row to `[WAITING TAG]` in the ledger.
- C5 TAG: run the audit yourself per `_internal/templates/tag_audit.md` — read the diff + every modified file end-to-end (no ellipsis/placeholders), apply architect-review/vibe-code-auditor/logic-lens/performance-optimizer/fix-review lenses, enforce WORM/RLS+secrets, emit `### TAG Batch 96 Evaluation Audit Report` matrix.
- C6 Resolve: if REJECTED → write TD+Findings+Kaizen, fix in-scope, re-run C2–C5 (out-of-scope fix → STOP). If PASSED → set ledger row `[PASSED TAG] Done`, commit `batch-96-packaging-proxmox`, open PR `Batch 96 — packaging-proxmox`.

---

**Skills:** `bash-scripting` (primary), `terraform-specialist`
**Depends on:** 95 · **Parallel:** —
**Files:** **new** `packaging/proxmox/` (LXC template or cloud-init VM image definition preloaded with the `.deb` from Batch 95 + a first-boot config wizard).

**Goal:** An appliance template with a first-boot wizard (enterprise ingest URL, credentials, namespace).

**Steps:**
1. LXC template (or cloud-init image) that installs the Batch-95 `.deb` on first boot.
2. First-boot config wizard collecting ingest URL + credentials + namespace and writing them to the env/`config.py` surface (never baked into the image).

**Acceptance:** documented appliance boot + first-run config + digest smoke test.

Final (self-orchestrated — do not skip): run the Closing Protocol C1–C6 above — gate (lint/typecheck for added Python + the documented appliance smoke test) → reviews → write `_internal/diffs/diff_batch_96-packaging-proxmox.md` + set ledger row `[WAITING TAG]` → run the TAG audit yourself per `_internal/templates/tag_audit.md` and emit the matrix → if REJECTED fix in-scope and re-run; if PASSED mark ledger Done, commit `batch-96-packaging-proxmox`, open PR `Batch 96 — packaging-proxmox`.
