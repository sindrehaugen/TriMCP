...
All reads happen in `nce/config.py` (the `_Config` class).
...
Pool acquire timeout is `10.0 s` (constant `POOL_ACQUIRE_TIMEOUT` in `nce/db_utils.py`).
...
|---|---|---|---|
| `NCE_MASTER_KEY` | `""` | **P0** | AES-256 master key for signing-key encryption at rest. **Server refuses to start** if absent or < 32 UTF-8 bytes. Minimum 32 random bytes, base64 or hex. |
| `NCE_API_KEY` | `""` | P1 | HMAC-SHA256 key for HTTP admin API auth (`HMACAuthMiddleware`). Warning logged if absent. |
| `NCE_ADMIN_USERNAME` | `""` | P1 | HTTP Basic Auth username for admin UI routes (`BasicAuthMiddleware`). |
| `NCE_ADMIN_PASSWORD` | `""` | P1 | HTTP Basic Auth password for admin UI routes. |
| `NCE_ADMIN_OVERRIDE` | `""` | **Dev only** | Set to `true` to bypass admin auth in development. **Raises `RuntimeError` at startup when `ENVIRONMENT=prod`.** |
| `ENVIRONMENT` | `dev` | P0 | Runtime environment label. Set to `prod` in production. Used by the admin-override guard (FIX-039). |
| `NCE_CLOCK_SKEW_TOLERANCE_S` | `300` | No | Maximum allowed timestamp drift (seconds) for HMAC replay protection. |
| `NCE_DISTRIBUTED_REPLAY` | `false` | No | Enable Redis-backed nonce store for HMAC replay protection across multiple admin replicas. |
...
| `NCE_JWT_PUBLIC_KEY` | `""` | RS256/ES256 PEM-encoded public key. May be a raw PEM string or `file:///path/to/pub.pem`. Takes precedence over `NCE_JWT_SECRET` when both are set. |
| `NCE_JWT_ALGORITHM` | `HS256` | One of `HS256`, `RS256`, `ES256`. |
| `NCE_JWT_ISSUER` | `""` | Expected `iss` claim. Empty = skip issuer check. |
| `NCE_JWT_AUDIENCE` | `""` | Expected `aud` claim. Empty = skip audience check. |
| `NCE_JWT_PREFIX` | `/api/v1/` | Route prefix protected by `JWTAuthMiddleware`. |
| `NCE_A2A_JWT_AUDIENCE` | `nce_a2a` | Per-service `aud` override for the A2A server, preventing token replay across system boundaries. |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_A2A_MTLS_ENABLED` | `false` | Master switch — enable mTLS client certificate enforcement on A2A routes. |
| `NCE_A2A_MTLS_STRICT` | `true` | Reject connections that present no client certificate. Set `false` for rolling deployments. |
| `NCE_A2A_MTLS_TRUSTED_PROXY_HOP` | `1` | Number of trusted reverse-proxy hops. `0` = direct TLS only; `1` = one proxy (Caddy / nginx). |
| `NCE_A2A_MTLS_ALLOWED_SANS` | `""` | Comma-separated allowed Subject Alternative Names (case-insensitive DNS/URI match). |
| `NCE_A2A_MTLS_ALLOWED_FINGERPRINTS` | `""` | Comma-separated allowed SHA-256 certificate fingerprints (colon-separated hex). |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_ADMIN_MTLS_ENABLED` | `false` | Enable mTLS client certificate enforcement on `/api/` admin routes. |
| `NCE_ADMIN_MTLS_STRICT` | `true` | Reject connections with no client certificate. |
| `NCE_ADMIN_MTLS_TRUSTED_PROXY_HOP` | `1` | Trusted reverse-proxy hops for `X-Forwarded-Client-Cert`. |
| `NCE_ADMIN_MTLS_ALLOWED_SANS` | `""` | Comma-separated allowed SANs for admin mTLS. |
| `NCE_ADMIN_MTLS_ALLOWED_FINGERPRINTS` | `""` | Comma-separated allowed SHA-256 fingerprints for admin mTLS. |
...
| Variable | Provider |
|---|---|
| `NCE_ANTHROPIC_API_KEY` | Anthropic Claude (`claude-opus-4-7`, etc.) |
| `NCE_OPENAI_API_KEY` | OpenAI (GPT-4.5, etc.) |
| `NCE_AZURE_OPENAI_API_KEY` | Azure OpenAI (`api-key` header) |
| `NCE_AZURE_OPENAI_ENDPOINT` | Azure OpenAI resource endpoint (required for `azure_openai` provider) |
| `NCE_AZURE_OPENAI_DEPLOYMENT` | Azure OpenAI default deployment name |
| `NCE_GEMINI_API_KEY` | Google AI Studio / Gemini |
| `NCE_DEEPSEEK_API_KEY` | DeepSeek (cost-sensitive deployments) |
| `NCE_MOONSHOT_API_KEY` | Moonshot / Kimi (large-context) |
| `NCE_OPENAI_COMPAT_BASE_URL` | Base URL for any OpenAI-compatible provider |
| `NCE_OPENAI_COMPAT_API_KEY` | API key for OpenAI-compatible provider |
| `NCE_OPENAI_COMPAT_MODEL` | Default model name for OpenAI-compatible provider |
| `NCE_LLM_PROVIDER` | `local-cognitive-model` | Default LLM provider label (matches labels in `providers/factory.py`). |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_COGNITIVE_BASE_URL` | `""` | Base URL for the local cognitive container (e.g. `http://cognitive:11435`). When set, embeddings route to `POST {base}/v1/embeddings`. |
| `NCE_COGNITIVE_EMBEDDING_MODEL` | `""` | Model name to request from the cognitive HTTP endpoint. |
| `NCE_COGNITIVE_API_KEY` | `""` | API key for the cognitive endpoint (optional). |
| `NCE_BACKEND` | `""` | Hardware backend selector: `openvino-npu` enables Intel NPU path via OpenVINO. |
| `NCE_OPENVINO_MODEL_DIR` | `""` | Path to the exported OpenVINO IR directory (required when `NCE_BACKEND=openvino-npu`). |
| `NCE_OPENVINO_SEQ_LEN` | `512` | Fixed token sequence length for the static NPU graph. |
| `NCE_OPENVINO_MODEL_REVISION` | `""` | HuggingFace commit SHA to pin when loading tokenizers with `trust_remote_code=True`. Warning logged if absent (FIX-053). |
...
### Webhook Receiver Secrets (nce/webhook_receiver)
...
| Variable | Default | Description |
|---|---|---|
| `NCE_SMTP_FROM` | `""` | Sender address for alert emails. **Required** — startup raises `ValueError` if empty when SMTP host is configured (FIX-052). |
| `NCE_SMTP_TO` | `""` | Recipient address for alert emails. **Required** (same condition as above). |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_QUOTAS_ENABLED` | `true` | Master switch — when `false`, quota queries are skipped on the tool hot path. |
| `NCE_QUOTA_TOKEN_ESTIMATE_DIVISOR` | `4` | Characters-per-token divisor for pre-flight quota estimates. |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_PROMETHEUS_PORT` | `8000` | Port for the Prometheus metrics scrape endpoint. |
| `NCE_OTEL_EXPORTER_OTLP_ENDPOINT` | `http://localhost:4318` | OpenTelemetry OTLP gRPC/HTTP exporter endpoint. |
| `NCE_OTEL_SERVICE_NAME` | `nce-python` | Service name reported to OTel backends. |
| `NCE_OBSERVABILITY_ENABLED` | `true` | Master switch for OTel + Prometheus instrumentation. |
...
| `NCE_DISABLE_MIGRATION_MCP` | `true` when `NCE_ENV` is production, else `false` | When `true`, `start_migration`, `commit_migration`, `abort_migration` are excluded from the MCP tool list. Set `false` explicitly in production only if you need live migration tools. |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_MAX_TEMPORAL_LOOKBACK_DAYS` | `90` | Maximum lookback window for `as_of` temporal queries (days). Prevents unbounded scans on `event_log`. Set `0` to disable. |
...
| Variable | Default | Description |
|---|---|---|
| `NCE_MAX_ATTACHMENT_BYTES` | `52428800` (50 MB) | Maximum accepted blob size for `extract_bytes` and `store_media`. Oversized payloads are rejected before any I/O. |
...
Processes `index_code_file` jobs from the `nce-tasks` Redis queue.
...
- Missing or too-short `NCE_MASTER_KEY`
- Empty `MINIO_ACCESS_KEY` or `MINIO_SECRET_KEY`
- Missing `MONGO_URI`, `PG_DSN`, or `REDIS_URL`

It logs `WARNING` (non-halting) when:
- `NCE_API_KEY` is absent (admin API inaccessible)
- Neither `NCE_JWT_SECRET` nor `NCE_JWT_PUBLIC_KEY` is set (A2A disabled)
