...
Database schema changes are managed via idempotent SQL scripts located in `nce/migrations/`. 

### Key Migrations
-   **`001_enable_rls.sql`**: Configures Row-Level Security and the `nce_app` role.
...
-   The `nce_app` role is granted `INSERT` and `SELECT` permissions but is explicitly denied `UPDATE` and `DELETE`.
