...
NCE maintains a tamper-evident audit trail for every memory and event. This is achieved through a mandatory HMAC-SHA256 signing layer that guarantees both integrity and causal provenance.
...
-   The server will **refuse to start** if `NCE_MASTER_KEY` is missing or less than 32 characters.
...
To ensure the signature is deterministic, NCE uses the **JSON Canonicalization Scheme (JCS)**. This ensures that even if keys in the JSON payload are reordered or whitespace changes, the resulting byte array used for signing remains identical.
