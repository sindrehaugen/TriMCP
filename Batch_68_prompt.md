...
1. One batch = one branch = one commit. Branch `batch-68-diag-queue-lane`. Never combine batches.
...
- C4 Diff: `git add -A` → write `git diff --cached` to `_internal/diffs/diff_batch_68-diag-queue-lane.md`; set this row to `[WAITING TAG]` in the ledger.
- C5 TAG: run the audit yourself per `_internal/templates/tag_audit.md` — read the diff + every modified file end-to-end (no ellipsis/placeholders), apply architect-review/vibe-code-auditor/logic-lens/performance-optimizer/fix-review lenses, enforce WORM/RLS+secrets, emit `### TAG Batch 68 Evaluation Audit Report` matrix.
- C6 Resolve: if REJECTED → write TD+Findings+Kaizen, fix in-scope, re-run C2–C5 (out-of-scope fix → STOP). If PASSED → set ledger row `[PASSED TAG] Done`, commit `batch-68-diag-queue-lane`, open PR `Batch 68 — diag-queue-lane`.

---

**Skills:** `python-pro` (primary), `backend-architect`
**Depends on:** none · **Parallel:** group A (65,66,67,68)
**Files:** `nce/extractors/dispatch.py` (lane constants `HIGH_PRIORITY_QUEUE`/`BATCH_QUEUE` ~`:25-31`, `get_queue_name` ~`:35`, `get_priority_queue` ~`:44`); `start_worker.py` (`QUEUE_NAMES` ~`:63`, `RecoveringWorker`/`start_worker` ~`:124-150`).

**Goal:** Add a dedicated `diag_ingest` RQ lane so heavy bundle jobs never starve normal batch jobs (P0 isolation).

**Steps:**
1. Add `DIAG_INGEST_QUEUE = "diag_ingest"` next to the existing lane constants. Add a small `get_diag_queue(connection) -> Queue` helper mirroring `get_priority_queue` (respect the same `is_async`/test-env handling).
2. Append `"diag_ingest"` to `QUEUE_NAMES` in `start_worker.py`. Add a comment that production should run a **separate worker process with `concurrency=1`** bound to this lane. Do not otherwise change the default worker behavior.

**Acceptance:** unit test asserting `get_diag_queue` returns a Queue named `diag_ingest` and the lane appears in `QUEUE_NAMES`. Pure-unit. `make lint && make typecheck` clean.

Final (self-orchestrated — do not skip): run the Closing Protocol C1–C6 above — gate (`make lint && make typecheck && pytest <the lane test>`) → reviews → write `_internal/diffs/diff_batch_68-diag-queue-lane.md` + set ledger row `[WAITING TAG]` → run the TAG audit yourself per `_internal/templates/tag_audit.md` and emit the matrix → if REJECTED fix in-scope and re-run; if PASSED mark ledger Done, commit `batch-68-diag-queue-lane`, open PR `Batch 68 — diag-queue-lane`.
