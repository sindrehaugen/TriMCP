...
--
-- Tables use public schema — no trimcp. prefix exists in this database.
...
--   INSERT/UPDATE/DELETE — trimcp_app may only touch rows in its own namespace.
--             WITH CHECK prevents trimcp_app from creating global (NULL) seeds;
...
CREATE POLICY tenant_isolation_policy ON query_templates
    ...
    USING  (namespace_id IS NULL OR namespace_id = get_trimcp_namespace())
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'trimcp_app') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE query_templates TO trimcp_app;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'trimcp_app') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE graph_schema_registry TO trimcp_app;
    END IF;
END $$;
