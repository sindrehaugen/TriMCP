...
@mcp_handler
async def handle_index_code_file(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
    ...
    if raw_bytes > cfg.TRIMCP_MAX_CODE_INDEX_BYTES:
        raise ValueError(
            f"raw_code exceeds maximum size: {raw_bytes} bytes "
            f"(limit: {cfg.TRIMCP_MAX_CODE_INDEX_BYTES} bytes). "
...
@mcp_handler
async def handle_check_indexing_status(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
...
@mcp_handler
async def handle_search_codebase(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
