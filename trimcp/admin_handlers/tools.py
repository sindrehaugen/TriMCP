...
logger = logging.getLogger("trimcp-admin-tools")
...
async def api_admin_tools(request) -> JSONResponse:
    ...
    try:
        if admin_state.engine.redis_client:
            keys_raw = await admin_state.engine.redis_client.hkeys("trimcp:tools:disabled")
...
async def api_admin_tools_toggle(request) -> JSONResponse:
    ...
    Saves state in Redis under a custom hash `trimcp:tools:disabled` for instant multi-worker sync.
    ...
    try:
        if admin_state.engine.redis_client:
            if enabled:
                ...
                await admin_state.engine.redis_client.hdel("trimcp:tools:disabled", tool_name)
                logger.info("Enabled %s tool/skill dynamically: %s", tool_type.upper(), tool_name)
            else:
                # Add to disabled list to disable it
                await admin_state.engine.redis_client.hset("trimcp:tools:disabled", tool_name, "1")
