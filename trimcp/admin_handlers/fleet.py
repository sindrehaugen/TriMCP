    ...
    try:
        ...
        try:
            ...
            cursor, keys = await admin_state.engine.redis_client.scan(
                cursor=0, match="trimcp:*", count=500
            )
            for k in keys:
...
async def api_admin_connectors_status(request):
    ...
    try:
        ...
        if _shared.cfg.TRIMCP_COGNITIVE_BASE_URL:
            import httpx

            try:
                async with httpx.AsyncClient(timeout=1.0) as client:
                    resp = await client.get(f"{_shared.cfg.TRIMCP_COGNITIVE_BASE_URL}/health")
        ...
        external_apis = {
            "openai_compatible_cognitive": {
                "endpoint": _shared.cfg.TRIMCP_COGNITIVE_BASE_URL or "not_configured",
                "configured": bool(_shared.cfg.TRIMCP_COGNITIVE_BASE_URL),
...
async def api_admin_datastores_save(request):
    ...
    if not _shared.cfg.TRIMCP_ALLOW_ADMIN_DOTENV_PERSIST:
        return JSONResponse(
            {
                "error": "Persisting datastore configuration to .env is disabled. "
                "Set TRIMCP_ALLOW_ADMIN_DOTENV_PERSIST=true for local development only."
            },
            status_code=403,
        )
