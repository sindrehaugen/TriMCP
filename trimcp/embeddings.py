...
def _load_sentence_transformer(device: str):
    ...
    trust = cfg.TRIMCP_EMBEDDING_TRUST_REMOTE_CODE
    revision = cfg.TRIMCP_EMBEDDING_MODEL_REVISION or None  # None = latest
...
def _load_openvino_npu_bundle(model_dir: str, seq_len: int):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_dir, trust_remote_code=cfg.TRIMCP_EMBEDDING_TRUST_REMOTE_CODE
    )
...
def validated_cognitive_base_url(raw: str | None = None) -> str:
    ...
    from trimcp.providers.base import validate_base_url

    base = (raw if raw is not None else cfg.TRIMCP_COGNITIVE_BASE_URL or "").strip().rstrip("/")
    if not base:
        return ""
...
class CognitiveRemoteBackend(EmbeddingBackend):
    ...
    Auth: optional ``TRIMCP_COGNITIVE_API_KEY`` sent as ``Bearer`` if non-empty.
    Fallback model: ``TRIMCP_COGNITIVE_FALLBACK_MODEL`` (from config, not os.getenv).
    """
    ...
    def _sync_embed_batch(self, texts: list[str]) -> tuple[list[list[float]], bool]:
        ...
        try:
            ...
            if is_rate_limit or is_timeout:
                ...
                fallback_model = cfg.TRIMCP_COGNITIVE_FALLBACK_MODEL
...
async def embed_batch(texts: list[str]) -> list[list[float]]:
    ...
    - ``TRIMCP_EMBED_MAX_BATCH_TEXTS``: hard cap on number of texts per call.
    - ``TRIMCP_EMBED_MAX_TEXT_CHARS``: each text is truncated to this length.
    ...
    if len(texts) > cfg.TRIMCP_EMBED_MAX_BATCH_TEXTS:
        raise ValueError(
            f"embed_batch called with {len(texts)} texts; "
            f"limit is {cfg.TRIMCP_EMBED_MAX_BATCH_TEXTS} (TRIMCP_EMBED_MAX_BATCH_TEXTS). "
            "Split the batch before calling."
        )

    # Per-text character truncation — prevents tokenizer surprises and cost spikes.
    max_chars = cfg.TRIMCP_EMBED_MAX_TEXT_CHARS
