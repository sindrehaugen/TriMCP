...
from trimcp.constants import MAX_TOP_K as _MAX_TOP_K
...
from trimcp.query_catalog import CatalogManager

_MAX_SCHEMA_LIMIT: int = 200  # upper bound for describe_schema limit parameter


@mcp_handler
async def handle_suggest_queries(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
...
@mcp_handler
async def handle_execute_query_template(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
...
@mcp_handler
async def handle_describe_schema(engine: TriStackEngine, arguments: dict[str, Any]) -> str:
