...
Each entry records:
  ...
  * whether the tool mutates state and should bump the cache generation counter
    (``mutation``)
  * whether the tool is gated by ``TRIMCP_DISABLE_MIGRATION_MCP`` (``migration``)
...
class ToolSpec:
    ...
    Attributes:
        ...
        migration:  When *True* the tool is gated by
                    ``cfg.TRIMCP_DISABLE_MIGRATION_MCP``; a disabled gate
...
TOOL_REGISTRY: dict[str, ToolSpec] = {
    ...
    # ------------------------------------------------------------------
    # Migration tools  (gated by TRIMCP_DISABLE_MIGRATION_MCP)
    # ------------------------------------------------------------------
...
#: applies the ``TRIMCP_DISABLE_MIGRATION_MCP`` gate separately.
...
#: Tools gated by ``cfg.TRIMCP_DISABLE_MIGRATION_MCP``.
