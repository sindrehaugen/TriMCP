...
def _build_smtp_config() -> dict:
    ...
    if not smtp_from or not smtp_to:
        raise ValueError(
            "TRIMCP_SMTP_FROM and TRIMCP_SMTP_TO must be set. No example.com defaults permitted."
        )

    return {
        "from": smtp_from,
        "to": smtp_to,
        "username": os.environ.get("TRIMCP_SMTP_USER", "").strip() or None,
        "password": os.environ.get("TRIMCP_SMTP_PASS", "").strip() or None,
    }
...
class NotificationDispatcher:
    ...
    async def _send_slack(self, title: str, message: str):
        if not self.slack_webhook:
            return
        from trimcp.net_safety import validate_extractor_url
