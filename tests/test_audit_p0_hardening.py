...
def test_assert_admin_override_raises_in_production() -> None:
    from trimcp.config import assert_admin_override_not_in_production, cfg

    with patch.object(cfg, "TRIMCP_ADMIN_OVERRIDE", True), patch.object(cfg, "IS_PROD", True):
        with pytest.raises(RuntimeError, match="TRIMCP_ADMIN_OVERRIDE"):
            assert_admin_override_not_in_production()


def test_validated_cognitive_base_url_allows_loopback() -> None:
    from trimcp.embeddings import cognitive_health_check_url, validated_cognitive_base_url
...
def test_validated_cognitive_base_url_rejects_metadata_host() -> None:
    from trimcp.embeddings import validated_cognitive_base_url
    from trimcp.providers.base import LLMProviderError
...
def test_webhook_client_ip_ignores_xff_without_trust_proxy() -> None:
    from trimcp.webhook_receiver.main import _client_ip

    request = MagicMock()
    request.headers = {"x-forwarded-for": "203.0.113.50"}
    request.client = MagicMock(host="10.0.0.5")

    with patch("trimcp.webhook_receiver.main.cfg") as mock_cfg:
        mock_cfg.TRIMCP_WEBHOOK_TRUST_PROXY = False
...
def test_webhook_client_ip_honors_xff_when_trust_proxy() -> None:
    ...
    with patch("trimcp.webhook_receiver.main.cfg") as mock_cfg:
        mock_cfg.TRIMCP_WEBHOOK_TRUST_PROXY = True
        assert _client_ip(request) == "203.0.113.50"


def test_validate_admin_credentials_requires_keys_in_production(monkeypatch) -> None:
    from trimcp.config import _Config

    monkeypatch.setattr(_Config, "IS_PROD", True)
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_API_KEY", "")
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_USERNAME", "admin")
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_PASSWORD", "$pbkdf2$abc")

    with pytest.raises(RuntimeError, match="TRIMCP_ADMIN_API_KEY"):
...
def test_validate_admin_credentials_rejects_plaintext_password_in_prod(
    ...
    monkeypatch.setattr(_Config, "IS_PROD", True)
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_API_KEY", "admin-api-key")
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_USERNAME", "admin")
    monkeypatch.setattr(_Config, "TRIMCP_ADMIN_PASSWORD", "plaintext")
...
async def test_datastores_save_rejects_when_persist_disabled() -> None:
    ...
    with (
        patch("trimcp.admin_state.engine", MagicMock()),
...
async def test_trigger_gc_hides_exception_detail_in_prod() -> None:
    ...
    with (
        ...
        patch("trimcp.admin_http_support.cfg") as mock_cfg,
    ):
