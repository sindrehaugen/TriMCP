...
2. **Verify before you act.** Pre-flight (2026-06-11): Batch 23's `persist_atms_invalidation` (atms.py:~318) already SOFT-DELETES (`valid_to=now()`) the invalidated memories AND their topology edges, and `merged`/`superseded` are valid resolution values in `contradiction_mcp_handlers.py` — BUT no re-queue / salience-restore logic exists for them (confirmed absent). So this batch's residual is: (a) `kg_edges` *triplets* extracted from a retracted memory (distinct from the topology edges Batch 23 already soft-deletes) — floor their confidence via the Batch 106 `origin_event_id`; and (b) the missing `superseded`/`merged` re-derivation. **Read `persist_atms_invalidation` and confirm it does NOT already touch `kg_edges` triplets by `origin_event_id`** before coding the flooring step; if it does, drop (a) and report.
