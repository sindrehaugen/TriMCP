...
1. One batch = one branch = one commit. Branch `batch-87-fdw-federation`. Never combine batches.
...
- C4 Diff: `git add -A` → write `git diff --cached` to `_internal/diffs/diff_batch_87-fdw-federation.md`; set this row to `[WAITING TAG]` in the ledger.
- C5 TAG: run the audit yourself per `_internal/templates/tag_audit.md` — read the diff + every modified file end-to-end (no ellipsis/placeholders), apply architect-review/vibe-code-auditor/logic-lens/performance-optimizer/fix-review lenses, enforce WORM/RLS+secrets, emit `### TAG Batch 87 Evaluation Audit Report` matrix.
- C6 Resolve: if REJECTED → write TD+Findings+Kaizen, fix in-scope, re-run C2–C5 (out-of-scope fix → STOP). If PASSED → set ledger row `[PASSED TAG] Done`, commit `batch-87-fdw-federation`, open PR `Batch 87 — fdw-federation`.

---

**Skills:** `postgresql` (primary), `postgresql-optimization`, `database-migration`
**Depends on:** 67 · **Parallel:** —
**Files:** **new** edge-only migration `nce/migrations/0NN_edge_fdw.sql` (use the next free number at apply time; clearly mark "edge deployment only"); **new** short edge bootstrap doc/script under `packaging/` or `_internal/` describing required replica connection settings. Reference the `kg_nodes`/`kg_edges` schema in `nce/schema.sql`.

**Goal:** Mount the enterprise read-replica `kg_nodes`/`kg_edges` as read-only foreign tables on the edge (ontological consistency).

**Steps:**
1. Define `postgres_fdw` server + user mapping + foreign tables for `kg_nodes`/`kg_edges` (read-only). Leave the enterprise side untouched.
2. Document required replica connection settings; **never** store replica credentials in app tables (env/secret store only).

**Acceptance:** `@pytest.mark.integration` (edge-flavored) verifying a read-only `<=>` query against the foreign table works and that writes to it are rejected. `make lint && make typecheck` clean.

Final (self-orchestrated — do not skip): run the Closing Protocol C1–C6 above — gate (`make lint && make typecheck && pytest -m integration <the FDW test>`) → reviews → write `_internal/diffs/diff_batch_87-fdw-federation.md` + set ledger row `[WAITING TAG]` → run the TAG audit yourself per `_internal/templates/tag_audit.md` and emit the matrix → if REJECTED fix in-scope and re-run; if PASSED mark ledger Done, commit `batch-87-fdw-federation`, open PR `Batch 87 — fdw-federation`.
