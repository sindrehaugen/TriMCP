...
1. One batch = one branch = one commit. Branch `batch-81-diag-cron-puller`. Never combine batches.
...
- C4 Diff: `git add -A` → write `git diff --cached` to `_internal/diffs/diff_batch_81-diag-cron-puller.md`; set this row to `[WAITING TAG]` in the ledger.
- C5 TAG: run the audit yourself per `_internal/templates/tag_audit.md` — read the diff + every modified file end-to-end (no ellipsis/placeholders), apply architect-review/vibe-code-auditor/logic-lens/performance-optimizer/fix-review lenses, enforce WORM/RLS+secrets, emit `### TAG Batch 81 Evaluation Audit Report` matrix.
- C6 Resolve: if REJECTED → write TD+Findings+Kaizen, fix in-scope, re-run C2–C5 (out-of-scope fix → STOP). If PASSED → set ledger row `[PASSED TAG] Done`, commit `batch-81-diag-cron-puller`, open PR `Batch 81 — diag-cron-puller`.

---

**Skills:** `python-pro` (primary), `backend-architect`
**Depends on:** 65, 80 · **Parallel:** —
**Files:** `nce/cron.py` (mirror `_d365_sync_tick` + `acquire_cron_lock` + APScheduler `add_job` registration + the startup-coro block).

**Goal:** A scheduled per-namespace pull tick that runs registered API source adapters.

**Steps:**
1. Add `async def _diag_pull_tick(pool)`: early-return unless `cfg.NCE_DIAG_ENABLED`; `acquire_cron_lock("diag_pull", ttl)`; scan `namespaces.metadata->'diag'->>'enabled'`; per namespace use `scoped_pg_session`, call each registered adapter's `fetch_and_stage`, enqueue `process_diag_bundle` per returned URI; throttled-alert on failure; release the lock in `finally`.
2. Register with `scheduler.add_job(_diag_pull_tick, "interval", minutes=…, id="diag_pull", coalesce=True)` and add to the startup-coro block, gated by the flag.

**Acceptance:** unit test (mock pool/redis) asserting the tick no-ops when disabled and acquires/releases the lock when enabled. `make lint && make typecheck` clean.

Final (self-orchestrated — do not skip): run the Closing Protocol C1–C6 above — gate (`make lint && make typecheck && pytest <the cron test>`) → reviews → write `_internal/diffs/diff_batch_81-diag-cron-puller.md` + set ledger row `[WAITING TAG]` → run the TAG audit yourself per `_internal/templates/tag_audit.md` and emit the matrix → if REJECTED fix in-scope and re-run; if PASSED mark ledger Done, commit `batch-81-diag-cron-puller`, open PR `Batch 81 — diag-cron-puller`.
