...
6. **Migrations:** NONE. `memories.derivation_depth` + its index were established by **Batch C0** (already in `main`). Verify it exists; do NOT add a migration or edit `schema.sql`. If absent, STOP — C0 was not merged.
...
**Files:** `nce/consolidation.py` (cluster input selection ~`:188-233`, derived-edge insert ~`:321-333`, abstraction row insert); `nce/config.py`; `nce/settings_registry.py`; `tests/test_consolidation_depth.py` (new)
...
**Steps:**
1. Verify `memories.derivation_depth` (SMALLINT, default 0) + index exist from C0. Backfill any pre-C0 rows via a recursive CTE over `derived_from` (depth from episodic roots) IF not already done. `derived_from` is a JSONB UUID array (confirmed) — query with `@> jsonb_build_array(...)`. (No DDL here.)
