...
> Diagnostic Log Digestion Engine · Operational hardening (last of the central-first release). Master plan: `_internal/work-docs/roadmaps/Diagnostic_Log_Digestion_Engine_Plan_2026-06-10.md`. Ledger: `_internal/Roadmaps/diagnostics_execution_ledger.md`.

## Operating rules (apply to this batch)
1. One batch = one branch = one commit. Branch `batch-84-landing-gc`. Never combine batches.
...
- C4 Diff: `git add -A` → write `git diff --cached` to `_internal/diffs/diff_batch_84-landing-gc.md`; set this row to `[WAITING TAG]` in the ledger.
- C5 TAG: run the audit yourself per `_internal/templates/tag_audit.md` — read the diff + every modified file end-to-end (no ellipsis/placeholders), apply architect-review/vibe-code-auditor/logic-lens/performance-optimizer/fix-review lenses, enforce WORM/RLS+secrets, emit `### TAG Batch 84 Evaluation Audit Report` matrix.
- C6 Resolve: if REJECTED → write TD+Findings+Kaizen, fix in-scope, re-run C2–C5 (out-of-scope fix → STOP). If PASSED → set ledger row `[PASSED TAG] Done`, commit `batch-84-landing-gc`, open PR `Batch 84 — landing-gc`.

---

**Skills:** `python-pro` (primary), `aws-serverless`
**Depends on:** 69 · **Parallel:** —
**Files:** `nce/garbage_collector.py` (orphan sweep ~`:337-430`, `_collect_minio_orphans` ~`:354`).

**Goal:** Reap landing objects for ingestions that never ran (belt-and-suspenders beyond the bucket TTL).

**Steps:**
1. Extend the existing sweep (or add a sibling routine) to delete landing-bucket objects whose `diag_ingestions` row is `FAILED`/absent and older than a configurable age; emit the existing orphan-cleanup metric. Keep it namespace-prefix aware and idempotent.

**Acceptance:** integration test seeding a stale landing object with no/failed row → asserts deletion; a fresh/in-progress object is preserved. `make lint && make typecheck` clean.

Final (self-orchestrated — do not skip): run the Closing Protocol C1–C6 above — gate (`make lint && make typecheck && pytest -m integration <the GC test>`) → reviews → write `_internal/diffs/diff_batch_84-landing-gc.md` + set ledger row `[WAITING TAG]` → run the TAG audit yourself per `_internal/templates/tag_audit.md` and emit the matrix → if REJECTED fix in-scope and re-run; if PASSED mark ledger Done, commit `batch-84-landing-gc`, open PR `Batch 84 — landing-gc`. (This closes the central-first release — confirm Batches 65–84 are all `[PASSED TAG] Done` before starting 85.)
