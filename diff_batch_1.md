...
@@ -13,45 +14,13 @@ logging.basicConfig(level=logging.INFO)
...
+### Run Unit Tests
+Unit tests use mocked pools (`FakeAsyncpgPool`) and do not require live Docker services:
+```bash
+pytest
 ```
...
index c25232d..10bc324 100644
--- a/nce/config.py
+++ b/nce/config.py
@@ -36,7 +36,7 @@ if _legacy_keys:
     _mapping = "\n".join(
...
+    )  # 20 MB default
+
+    NCE_MAX_OCR_PAGES: int = _int_env(
+        "NCE_MAX_OCR_PAGES", 10, minimum=1
+    )
+
 
     # --- MCP Sizing Limits ---
     NCE_MAX_ARGUMENTS_JSON_SIZE: int = _int_env(
@@ -255,6 +260,9 @@ class _Config:
     NCE_MAX_METADATA_LIST_ITEMS: int = _int_env(
         "NCE_MAX_METADATA_LIST_ITEMS", 256, minimum=1
     )
+    NCE_MAX_CONCURRENT_TOOLS: int = _int_env(
+        "NCE_MAX_CONCURRENT_TOOLS", 16, minimum=1
+    )
 
     # --- Temporal queries ---
     # Maximum lookback window for ``as_of`` temporal queries.  Prevents
@@ -276,7 +284,7 @@ class _Config:
...
+    # --- Dynamics 365 / Dataverse vertical module ---
+    NCE_D365_ENABLED: bool = os.getenv("NCE_D365_ENABLED", "false").strip().lower() in ("1", "true", "yes")
...
+    NCE_D365_NETBOX_BRIDGE_ENABLED: bool = (
+        os.getenv("NCE_D365_NETBOX_BRIDGE_ENABLED", "false").strip().lower() in ("1", "true", "yes")
+    )
...
@@ -107,16 +106,43 @@ class ConsolidationWorker:
...
index 0508a79..78a4d7c 100644
--- a/nce/cron.py
+++ b/nce/cron.py
@@ -311,12 +311,168 @@ async def _reembedding_tick(pool: asyncpg.Pool, mongo_client: Any) -> None:
...
@@ -423,6 +579,30 @@ async def async_main() -> None:
...
@@ -445,15 +625,20 @@ async def async_main() -> None:
...
index 9a327a7..338c3c2 100644
--- a/nce/db_utils.py
+++ b/nce/db_utils.py
@@ -30,6 +30,9 @@ UNMANAGED_PG_AUDITED_SITES: Final[frozenset[str]] = frozenset(
...
@@ -89,9 +92,7 @@ async def scoped_pg_session(
...
@@ -885,6 +913,16 @@ async def _resolve_llm_payload(
...
+addopts = -p no:unraisableexception -W ignore::DeprecationWarning
...
@@ -30,6 +27,9 @@ from nce.event_log import (
 )
...
-
 ...
 from nce.replay import (
     ForkedReplay,
@@ -62,3 +61,182 @@ async def test_replay_mode_error_on_invalid_llm_mode() -> None:
