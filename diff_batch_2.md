...
@@ -691,6 +719,10 @@ async def _handle_consolidation_run(
     if not abstraction:
...
@@ -703,15 +735,13 @@ async def _handle_consolidation_run(
         """
         INSERT INTO memories (
             id, namespace_id, agent_id,
-            summary, embedding,
...
+            $5, $6::jsonb,
         ...
         ON CONFLICT DO NOTHING
@@ -719,9 +749,8 @@ async def _handle_consolidation_run(
         new_memory_id,
         target_ns,
         src.agent_id,
-        abstraction,
...
@@ -732,6 +761,23 @@ async def _handle_consolidation_run(
         ),
     )
...
@@ -885,6 +931,16 @@ async def _resolve_llm_payload(
